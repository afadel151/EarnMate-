...
async function sendBybit() {
        ...
        fd.append("amount", parseFloat(offer.value.required_amount))
        try {
            const response = await axios.post("/api/offers/bybit", fd);
            if (response.data == 'done') {
                toast.add({ severity: 'success', summary: 'Info', detail: 'Deposited successfully', life: 3000 });
            }
            visible.value = false;
...
async function sendBaridi() {
    if (priceStore.price != 0) {
        ...
        toast.add({ severity: 'error', summary: 'Info', detail: 'Cant use this method right now', life: 3000 });
    }
            ...
            <TabPanels>
                ...
                <TabPanel v-if="props.offer.method === 'binance' || props.offer.method === 'all'" value="1" as="p"
                    ...
                    <div class="flex items-center gap-4">
                        <label for="email" class="font-semibold w-24">Amount</label>
                        <InputNumber required readonly v-model="requiredAmount"  mode="currency" currency="USD"
